...
def get_feature(train, features, cate_feat):
    ...
    data['is_worktime'] = list(
        map(lambda x, y: 1 if (y <= 6) and ((x >= 8 and x <= 12) or (x >= 14 and x <= 18)) else 0, data['hour'],
            data['weekday']))
    ...
    data['is_eat'] = data['hour'].map(
        lambda x: 1 if (x >= 7 and x <= 9) or (x >= 12 and x <= 13) or (x >= 17 and x <= 18) else 0)
    ...
    tmp['Date_Hour'] = list(map(lambda x, y: 2020020100 + x % 100 + (int((x % 10000) / 100) - 32) * 100 if int(
        (x % 10000) / 100) >= 32 and y == 1 else x, tmp['Date_Hour'], tmp['month']))
    ...
    tmp['is_week_ratio_week'] = list(
        map(lambda x, y: 0 if y == 0 else x / y, tmp['is_week_sum_Index'], tmp['week_sum_Index']))
...
def solve():
    ...
    info_data = info_data.rename(
        columns={0: 'ID', 1: 'area_name', 2: 'area_type', 3: 'Center_x', 4: 'Center_y', 5: 'Grid_x', 6: 'Grid_y',
                 7: 'area'})
    ...
    migration_index = migration_index.rename(
        columns={0: 'Date_Hour', 1: 'departure_province', 2: 'departure_city', 3: 'arrival__province',
                 4: 'arrival_city', 5: 'index'})
    migration_index['out_index'] = list(
        map(lambda x, z: z if x == '北京市' else 0, migration_index['departure_province'], migration_index['index']))
    migration_index['in_index'] = list(
        map(lambda x, z: z if x == '北京市' else 0, migration_index['arrival__province'], migration_index['index']))
    pivot = pd.pivot_table(migration_index, values=['out_index', 'in_index'], index=['Date_Hour'],
    ...
    tmp = pd.DataFrame(tmp).rename(
        columns={'in_index': 'week_mean_in_index', 'out_index': 'week_mean_out_index'}).reset_index()
    ...
    features = ['weekday', 'month', 'day', 'hour', 'area_type_1', 'area_type_2', 'Center_x', 'Center_y', 'Grid_x',
                'Grid_y', 'area']
    ...
    model.fit(train_data[features], train_data['Index'])
...
def solve2():
    ...
    for day in range(1, 10):
        ...
        train_data = data[(data['Date_Hour'] >= 2020020800) & (data['Date_Hour'] <= 2020021523)]
        train_pred = data[
            (data['Date_Hour'] >= (20200215 + day) * 100) & (data['Date_Hour'] <= (20200215 + day) * 100 + 23)]
        ...
        out.loc[
            (out['Date_Hour'] >= (20200215 + day) * 100) & (out['Date_Hour'] <= (20200215 + day) * 100 + 23), 'Index'] = \
        train_pred['pred'].values
        use_data.loc[(use_data['Date_Hour'] >= (20200215 + day) * 100) & (
                    use_data['Date_Hour'] <= (20200215 + day) * 100 + 23), 'Index'] = train_pred['pred'].values
...
def solve3():
    ...
    out.loc[(out['Date_Hour'] >= 2020021600) & (out['Date_Hour'] <= 2020021623), 'Pred'] = out.loc[(out[
                                                                                                        'Date_Hour'] >= 2020021600) & (
                                                                                                               out[
                                                                                                                   'Date_Hour'] <= 2020021623), 'Pred'] * 0.95
    out.loc[(out['Date_Hour'] >= 2020022100) & (out['Date_Hour'] <= 2020022123), 'Pred'] = out.loc[(out[
                                                                                                        'Date_Hour'] >= 2020022100) & (
                                                                                                               out[
                                                                                                                   'Date_Hour'] <= 2020022123), 'Pred'] * 1.02
    out.loc[(out['Date_Hour'] >= 2020022400) & (out['Date_Hour'] <= 2020022423), 'Pred'] = out.loc[(out[
                                                                                                        'Date_Hour'] >= 2020022400) & (
                                                                                                               out[
                                                                                                                   'Date_Hour'] <= 2020022423), 'Pred'] * 1.05
    out.to_csv('./result/block_predict_result.csv', header=None, sep=',', index=False)
